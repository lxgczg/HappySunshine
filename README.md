...
## 2、离线抽取
![HS](https://github.com/lxgczg/HappySunshine/blob/main/Photo/HsPgUnload%E8%A7%A3%E6%9E%90%E5%9B%BE.png)
PG数据离线抽取功能是一个多线程程序，解析流程如上。

# 四、升级点
序号|名称|备注
-- | ----- | ------ 
1|修复坏页场景下多线程卡死BUG	    |PG数据离线抽取功能，单线程正常。
2|TUPLE损坏判断逻辑优化	        |PG数据离线抽取功能。
3|TUPLE可见性判断逻辑优化	    |PG数据离线抽取功能。
4|空字符串处理优化	|PG数据离线抽取功能。
5|COPY语句自动生成	    |PG数据离线抽取功能。
6|添加正常页、坏页统计	    |PG数据离线抽取功能。
7|提升坏页场景下程序健壮性    |PG数据离线抽取功能。
